...
Edits persist in SQLite (`edits.db`, keyed by content hash, each row stamped `updated_at`), optionally mirrored to `.negpy` JSON sidecars next to sources (`negpy/services/assets/sidecar.py`). A sidecar is a format-3 envelope (`saved_at`, `edit`, `mark`, `work_prints`, `roll_locks`); the mirror writes it with the row's own `updated_at`, so a mirrored file never reads as newer than its row. A mark, work-print or roll-lock change lives in its own table but advances that `updated_at` (`touch_file_settings`), so it reaches a machine that already has the frame. `roll_locks` is the frame's lock set in the folder roll holding its path, null for a fork; a file without it (format 2) locks each card that differs from the roll's defaults on load. Roll ids are per machine, so a baseline source naming the frame's folder roll is written `roll:` and read back as the folder roll here. A folder roll mirrors to `<folder>/.negpy-roll` (`rolls.PORTABLE_FIELDS` plus half-frame mode; never paths, forks or locks), stamped with the roll's own `updated_at` and never written over a file saved after that stamp: every discovery reads the files of the folder rolls its paths are in before it starts (`controller._read_roll_sidecars`), a roll that holds nothing here adopts it silently (`rolls.adopts_roll_file`; a folder whose roll was deleted is offered it instead), and a newer one joins the frames' offer. On load the DB wins; a sidecar fills a DB miss, and one saved after the row is offered once per folder open (`controller._offer_newer_sidecars`) or pulled explicitly with Reload from Sidecar.

...
**Folder moves** (`services/assets/repoint.py`) — `repoint_folder(repo, old, new)` is the one routine that rewrites every path stored under a folder: rolls' `folder_path`/`extra_paths`/`member_paths`, `composites_by_path`, the saved session, import sources, dismissed folders, library roots, the `file_path` columns of edits, marks and embeddings, and the source paths inside saved configs (without touching `updated_at`); `AppController.repoint_folder` adds the loaded assets (`session.rehome_folder_paths`). A disk rename (`AppController.request_rename_roll`) flushes the mirror first and repoints after. Another computer follows, and never touches the disk: recognition (`recognize_roll_folder`, used by import, Library refresh and open) and discovery hand a folder whose roll file names a roll here whose folder is gone, by `roll_uid` or by a former folder name in the same parent, to `follow_folder`, which repoints that roll and keeps its id; a copy, whose roll's folder is still there, becomes its own roll with an unwritten uid its file takes on the next write. Open and restore look for a missing folder first (`find_moved_folder`: siblings, then import sources and library roots, reading only roll files).

...
Edits persist in SQLite (`edits.db`, keyed by content hash, each row stamped `updated_at`), optionally mirrored to `.negpy` JSON sidecars next to sources (`negpy/services/assets/sidecar.py`). A sidecar is a format-3 envelope (`saved_at`, `edit`, `mark`, `mark_at`, `work_prints`, `roll_locks`); the mirror writes `saved_at` from the row's own `updated_at` and `mark_at` from the mark's `marked_at`, so a mirrored file never reads as newer than its rows. A roll-lock change advances `updated_at` (`touch_file_settings`). A frame with a mark or work print but no edit gets a file with a null `edit` and `saved_at`; before writing, the mirror takes the file's newer mark and work prints and, with no edit here, its edit (`load_or_promote`), so it never writes older state over the file's. Every discovery takes a mark dated after the one here and, per work-print name, the later of a save and a deletion or rename away (`merge_sidecar_extras`; tombstones in `work_print_tombstones`, `deleted_work_prints` in the file), whatever the edit; a cleared mark keeps its row, so the clear travels. Discovery scans the frames' files on the discovery thread (`workers/sidecar_scan.py`; reads only, one parse per changed (mtime_ns, size), a config built only for what is applied or offered); the UI thread writes the result, re-checked against the DB, then shows the offer, and drops a superseded generation. `roll_locks` is the frame's lock set in the folder roll holding its path, null for a fork; a file without it (format 2) locks each card that differs from the roll's defaults on load. Roll ids are per machine, so a baseline source naming the frame's folder roll is written `roll:` and read back as the folder roll here. A folder roll mirrors to `<folder>/.negpy-roll` (`rolls.PORTABLE_FIELDS` plus the Half Frame and Trichrome modes and the roll's own half-frame split profile; never paths, forks, locks or per-scan split overrides), stamped with the roll's own `updated_at` and never written over a file saved after that stamp: every discovery reads the files of the folder rolls its paths are in before it starts (`controller._read_roll_sidecars`), a roll that holds nothing here adopts it silently (`rolls.adopts_roll_file`; a folder whose roll was deleted is offered it instead), and a newer one joins the frames' offer. The file also carries the roll's identity, under its own rules (`sidecar.plan_roll_file_write`): `roll_uid` (made by the first write; a read takes the file's unless another roll here holds it), the name's leaf with `name_at` (a rename's `name_updated_at`, which never stamps the roll; the later wins on every read, apart from adopt or offer, and replaces only the leaf here) and `former_names` (folder names a disk rename left). Over a newer file the mirror patches only those, a roll never changed here writes them alone with a null `saved_at`, and a file that cannot be read is never written over. On load the DB wins for the edit; a sidecar fills a DB miss, a null edit never creates or replaces one, and an edit saved after the row is offered once per folder open (`controller._on_sidecar_scan_planned` → `_show_sidecar_offers`) or pulled explicitly with Reload from Sidecar.
...
**Composite membership** (`services/assets/composites.py`) — which files a stitch or an HDR merge is made of is a user decision that nothing in the files records, so it is stored per primary path and lives until the composite is dissolved, not until the file list changes. Every asset discovery re-attaches from it and drops the parts it consumed. `_persist_session` upserts, never rewrites: the open files are one folder, the store is all of them. RGB-scan triplets have their own store (`services/assets/rgb_triplets.py`), read only while RGB Scan is on and kept in line with the open files: it saves a re-measurement, and turning the mode off still ungroups.

**Rolls** (`services/assets/rolls.py`) — a Roll is a named, openable group of frames and the Library's only unit: a recognized folder, or a virtual roll built by hand from whatever the Film Strip holds. Folders are an import mechanism, not a live view. A roll also carries its own Half Frame and Trichrome state (`half_frame_mode_by_roll`, `rgbscan_mode_by_roll`) and roll-wide defaults for the Film Mode, Calibration, Crop, Roll Analysis, Metering, Raw Decode, Lens, Flat Field and Metadata cards (`ROLL_DEFAULT_FIELDS`, one `(config section, fields)` pair per card, so a card is not tied to `ProcessConfig`; the Optics section drives Lens and Flat Field through `controls_panel._SECTION_CARDS`, one scope pair over both); a frame that diverges is *locked* on that card until an Apply pushes it back out (`AppController.apply_roll_cards_to_roll` / `_to_selected`). What a card holds is the roll's or the rig's — the film edge, the scanning lens, the light, the stock's White/Black Point — never one frame's own placement: the crop rectangle, the rotation, the easel movements and the capture frame number stay per-frame. Every section header carries the same Frame/Roll pair (`CollapsibleSection.set_scope_buttons`), whose Roll half only pushes, and a Reset to Roll button that pulls the roll's value back (`set_roll_revert`, `AppController.revert_to_roll`); a frame-level card has no roll default and reads Roll from `rolls.section_push`, a record of what a whole-roll apply put there, which never overlays onto another frame. A roll also holds **scenes** (`rolls.roll_scenes`): hand-picked groups keyed by unforked hash, one scene per frame, each with its own normalization baseline that Roll Analysis skips.

**Folder moves** (`services/assets/repoint.py`) — `repoint_folder(repo, old, new)` rewrites every path stored under a folder: rolls, `composites_by_path`, the saved session, import sources, dismissed folders, library roots, the `file_path` columns, and source paths inside saved configs, keeping `updated_at`; `AppController.repoint_folder` adds the loaded assets and the mirror's queue. A disk rename flushes the mirror first. Another computer follows without touching the disk: `roll_moved_to` reads a new folder's roll file, finds its roll here by `roll_uid` or a former folder name in the same parent, and returns `MOVED` (that roll's folder is gone from a parent that is there; `follow_folder` repoints it and keeps its id), `COPY` (another folder, by `samefile`, whose file holds the same uid; a new roll with an unwritten uid) or `UNSURE` (another path to one folder, an offline share, a stale listing; left for the next pass). Open and restore search for a missing folder (`find_moved_folder`: siblings, then the library for a roll with a uid), once a session per roll.
...
- **Shortcuts** (`negpy/desktop/view/shortcut_registry.py`) — `REGISTRY` is the single source of truth for every binding: one `ShortcutEntry(default_key, description, category)` per action id, dispatched through the matching entry in the action map in `keyboard_shortcuts.py`. It also feeds the shortcut editor, the `?` overlay and `tooltip_with_shortcut()`.
  **Any new user-facing toggle, tool or action gets a registry entry** — leave `default_key` empty rather than inventing a conflicting one. Check for collisions before picking: the same key on two actions makes Qt fire `activatedAmbiguously` and both go dead. An entry's `scope` names the window that binds it; a key is unique within a scope, and a scoped window takes its keys ahead of the main window's (`LiveViewWindow.set_shortcuts`). `docs/KEYBOARD.md` is generated, so run `uv run python -m negpy.desktop.view.keyboard_doc` after a registry change. Copy that names a key reads it through `key_for`/`label_with_shortcut`, never as a literal.

...
The roll file also names the roll, with one id on every computer, and carries its display name. A rename reaches the other computers without a dialog, even after **Keep Mine**; the later rename wins. When the folder is renamed on one computer, the others follow it on **↻**, import or open: the roll keeps its settings, scenes, card locks, independent edits, stitches, HDR merges and virtual-roll members, and they never rename anything on disk. A copied folder becomes a roll of its own. With Keep Current off nothing is written: a roll whose file already named it still follows its folder, but a new display name stays on this computer, and a roll whose file did not name it yet comes back as a new roll beside the old one, which shows **folder missing**.
